...
public class Stars : MonoBehaviour, IPlanet {
    ...
    private void Awake()
    {
        if (StarBackground == null)
            Debug.LogError("StarBackground is not assigned in the Inspector.", this);
        if (Star == null)
            Debug.LogError("Star is not assigned in the Inspector.", this);
        if (StarFlares == null)
            Debug.LogError("StarFlares is not assigned in the Inspector.", this);

        var starBgImage = StarBackground?.GetComponent<Image>();
        var starImage = Star?.GetComponent<Image>();
        var starFlaresImage = StarFlares?.GetComponent<Image>();

        if (starBgImage == null)
            Debug.LogError("StarBackground does not have an Image component.", StarBackground);
        if (starImage == null)
            Debug.LogError("Star does not have an Image component.", Star);
        if (starFlaresImage == null)
            Debug.LogError("StarFlares does not have an Image component.", StarFlares);

        m_Starbackground = starBgImage?.material;
        m_Star = starImage?.material;
        m_StarFlares = starFlaresImage?.material;
